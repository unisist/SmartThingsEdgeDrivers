...
local NEO_COOLCAM_FINGERPRINTS = {
  {mfr = 0x0258, prod = 0x0003, model = 0x0087} -- Neo Coolcam Metering SmartPlug
}

local function can_handle_metering_smartplug(opts, driver, device, ...)
  log.info(string.format(" Neo Coolcam plug handle!!"))
  for _, fingerprint in ipairs(NEO_COOLCAM_FINGERPRINTS) do
...
local metering_smartplug = {
  NAME = "Neo Coolcam Metering SmartPlug",
  capability_handlers = {
  ...
  zwave_handlers = {
    [cc.METER] = {
      [Meter.REPORT] = meter_report_handler1
    }
  },
  can_handle = can_handle_metering_smartplug,
}

return metering_smartplug
